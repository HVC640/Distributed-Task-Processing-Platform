...
def execute_task(task_id, WORKER_ID):
    try:
        ...
        if not claim_task(task_id, WORKER_ID):
            logger.warning(
                "Failed to claim task, it may have been claimed by another worker",
                extra={"extra_data": {"worker_id": WORKER_ID, "task_id": task_id}},
            )
            return
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                if not is_still_owner(task_id, WORKER_ID):
                    logger.warning(
                        "Lost ownership of task, skipping completion",
                        extra={
                            "extra_data": {"worker_id": WORKER_ID, "task_id": task_id}
                        },
                    )
                    return
                ...
                update_task_status(
                    task_id,
                    "FAILED",
                    f"Timeout after {WORKER_CONFIG['worker_timeout']} seconds",
                )
        ...
        logger.info(
            "Task removed from processing queue",
            extra={"extra_data": {"worker_id": WORKER_ID, "task_id": task_id}},
        )
        end_time = time.time()
