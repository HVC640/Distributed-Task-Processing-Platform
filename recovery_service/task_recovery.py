...
def recover_task(task_id):
    logger.info(f"Attempting to recover task", extra={
                "extra_data": {"task_id": task_id}})
    task = get_task_by_id(task_id)

    if not should_recover(task['lease_expires_at']):
        logger.info(f"Task is not ready for recovery",
                    extra={"extra_data": {"task_id": task_id}})
        return

    logger.info(f"Recovering task", extra={"extra_data": {
                "task_id": task_id, "lease_expires_at": task['lease_expires_at']}})
    ...
    if retry_count >= max_retries:
        logger.info(f"Task has exceeded max retries",
                    extra={"extra_data": {"task_id": task_id}})
    ...
    logger.info(f"Task re-enqueued for processing",
                extra={"extra_data": {"task_id": task_id}})
